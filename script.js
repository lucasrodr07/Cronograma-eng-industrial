...
let configGeral = { dataInicio: dayjs().startOf('year').format('YYYY-MM-DD'), dataFim: dayjs().endOf('year').format('YYYY-MM-DD') };
let projetosLocais = [];
let areasLocais = {};
let tarefas = [];
...
db.collection("settings").doc("global").onSnapshot((doc) => {
    if (!doc.exists && doc.metadata.fromCache) {
        document.getElementById('data-header').innerText = `Aguardando conexão com o servidor...`;
        return;
    }

    if (doc.exists) {
        const data = doc.data();
        configGeral = data.config || configGeral;
