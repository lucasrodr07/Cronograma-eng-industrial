...
db.collection("settings").doc("global").onSnapshot((doc) => {
    ...
    initAnoSelector();
    if(tarefasCarregadas) processarDados();
});
...
db.collection("tarefas").onSnapshot((snap) => {
    ...
    if(configCarregada) processarDados();
});
...
function processarDados() {
    ...
    // Rola para hoje imediatamente no primeiro carregamento
...
function irParaHoje(forceChangeYear = false) {
    ...
    if(parseInt(sel.value) !== anoH) {
        if(forceChangeYear) {
            ...
            trocarAno();
            setTimeout(() => irParaHoje(false), 400);
        }
        return;
    }
...
// SETUP RESTAURADO PARA O VISUAL ORIGINAL APROVADO
function renderizarListasConfig() {
    const selAno = parseInt(document.getElementById('filtro-ano-view').value) || Math.max(2026, dataSistema.year());
    document.getElementById('setup-title').innerText = `⚙️ Setup de Projetos (${selAno})`;
