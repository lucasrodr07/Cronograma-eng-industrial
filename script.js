...
const i18n = {
    'pt-br': {
        ...
        strAllProj: "Todos os Projetos", strAllArea: "Todas as Áreas", strAllResp: "Todos os Responsáveis", strNoSector: "Sem Setor", strNoData: "Nenhuma ação encontrada.",
...
window.abrirModalAcaoRapida = function(projeto, area, inicio) {
    abrirModalAcao(null);
    document.getElementById('i-projeto').value = projeto;
    atualizarAreasDoModalAcao();
    document.getElementById('i-area').value = area;
    document.getElementById('i-inicio').value = inicio;
    document.getElementById('i-fim').value = inicio;
}
...
window.scrollToMonthName = function(monthStr) {
    const ths = document.querySelectorAll('.head-mes th');
    for(let th of ths) {
        if(th.innerText.trim() === monthStr) {
            const container = document.getElementById('gantt-container');
            container.scrollTo({ left: th.offsetLeft - 150, behavior: 'smooth' });
            break;
        }
    }
};
...
function renderizarGantt(startStr, endStr, tituloLabel) {
    ...
    if (projsVisiveis.length === 0) { 
        h += `<tr><td colspan="10" style="padding:40px; text-align:center;"><div class="empty-state"><span>📭</span>${dict.strNoData}</div></td></tr>`; 
    }
            ...
            exibe.forEach(areaNome => {
                ...
                camadas.forEach((camada, cIdx) => {
                    ...
                    dArr.forEach(d => {
                        ...
                        h += `<td class="${(d.s === 0 || d.s === 6) ? 'is-weekend' : ''} ${d.ds === hojeReal ? 'is-today' : ''} ${isMonthEnd ? 'month-end' : ''}" ondragover="allowDrop(event)" ondrop="dropTask(event,'${p}','${areaNome}','${d.ds}')" ondblclick="abrirModalAcaoRapida('${p}', '${areaNome}', '${d.ds}')">`;
                        ...
                        if(task){
                            ...
                            h += `<div class="task-wrapper ${clDimmed}" id="task-${task.id}" draggable="true" ondragstart="dragStart(event, '${task.id}')" onclick="handleTaskClick(event, '${task.id}')" ondblclick="event.stopPropagation()" oncontextmenu="abrirContextMenu(event, '${task.id}')" onmousemove="showTooltip(event,'${task.id}')" onmouseleave="hideTooltip()" style="width:calc(${dur*100}% + ${dur-1}px);">
    ...
    // INJEÇÃO DA LINHA FANTASMA (SCROLL BUFFER) PARA O DOCK NÃO COBRIR TAREFAS
    h += `<tr class="no-print"><td colspan="${dArr.length+2}" style="height: 70px; border: none; background: transparent; pointer-events: none;"></td></tr>`;
    
    h += `</tbody></table>`;
    document.getElementById('gantt-container').innerHTML = h;
    let elHeader = document.getElementById('data-header');
    if(elHeader) elHeader.innerText = `${dict.title} - ${tituloLabel} | `;

    if (escala !== 'mensal') {
        let dockHtml = ``;
        let uniqueMonths = [...new Set(dArr.map(d => d.m.substring(0,3).toUpperCase()))];
        uniqueMonths.forEach(m => {
            dockHtml += `<div class="dock-month" onclick="scrollToMonthName('${m}')">${m}</div>`;
        });
        const dockElement = document.getElementById('timeline-dock');
        if(dockElement) dockElement.innerHTML = dockHtml;
    }
}
...
function showTooltip(e, id) {
    ...
    if(t.subtarefas && t.subtarefas.length > 0) {
        ...
        t.subtarefas.forEach(s => { htmlSub += `<span style="${s.feita ? 'color:#888; text-decoration:line-through;' : ''}"><br>${s.feita ? '✅' : '⬜'} ${s.texto}</span>`; });
        htmlSub += `</div>`;
    }
...
function removerP(n) {
    ...
    let msg = `Remover o projeto "${n}" do ano ${selAno}?`;
    if(tProj.length > 0) {
        msg = `ATENÇÃO: O projeto "${n}" possui ${tProj.length} ação(ões) vinculada(s).\n\nSe você remover o projeto, TODAS estas ações serão excluídas permanentemente do banco de dados.\n\nDeseja realmente excluir o projeto e todas as suas ações?`;
    }
    if(confirm(msg)) {
        let promises = tProj.map(t => db.collection("tarefas").doc(t.id).delete());
...
function removerA(p, i) {
    ...
    if(confirm(msg)) {
        let promises = tArea.map(t => db.collection("tarefas").doc(t.id).delete());
        Promise.all(promises).then(() => {
...
function gerarRelatorioImpressao(startStr, endStr, escala) {
    ...
    let hTable = `<div style="margin-top: 40px; width: 100%; font-family: 'Graphik', Arial, sans-serif; page-break-before: auto;">
        ...
        <table class="print-appendix-table" style="width: 100%; border-collapse: collapse; font-size: 10px; border: 2px solid #333;">
            <thead>
        ...
        sortedTasks.forEach(t => {
            ...
            hTable += `<tr style="border-bottom: 1px solid #888; page-break-inside: avoid;">
                <td style="padding: 6px; border: 1px solid #d4d4d4; text-align: center;">
                    <div style="width: 12px; height: 12px; background: ${statusColor}; border-radius: 3px; margin: 0 auto; border: 1px solid #333;"></div>
                </td>
                <td style="padding: 6px; border: 1px solid #d4d4d4; font-weight: bold; word-break: break-word; max-width: 180px; white-space: normal;">${t.texto}</td>
