...
// NOVO: Adicionado as Chaves do Heatmap no Motor de Tradução
const i18n = {
    'pt-br': {
        ...
        cmComplete: "✅ Concluir (100%)", cmAtt: "⚠️ Marcar Atenção", cmNorm: "ℹ️ Status Normal", cmDel: "🗑️ Excluir Ação"
    },
    'en': {
        ...
        cmComplete: "✅ Complete (100%)", cmAtt: "⚠️ Mark Attention", cmNorm: "ℹ️ Normal Status", cmDel: "🗑️ Delete Task"
    },
    'de': {
        ...
        cmComplete: "✅ Abschließen (100%)", cmAtt: "⚠️ Achtung markieren", cmNorm: "ℹ️ Normaler Status", cmDel: "🗑️ Aufgabe löschen"
    },
    'es': {
        ...
        cmComplete: "✅ Completar (100%)", cmAtt: "⚠️ Marcar Atención", cmNorm: "ℹ️ Estado Normal", cmDel: "🗑️ Eliminar Tarea"
    }
};
...
document.getElementById('gantt-container')?.addEventListener('scroll', () => { document.getElementById('context-menu').style.display = 'none'; });

function cmAcao(acao) {
    if(!currentContextId) return;
    const t = tarefas.find(x => x.id === currentContextId);
    if(!t) return;
    
    if(acao === 'concluir') {
        db.collection('tarefas').doc(currentContextId).update({ progresso: 100 });
    } else if(acao === 'suporte') {
        db.collection('tarefas').doc(currentContextId).update({ status: 'suporte' });
    } else if(acao === 'informativo') {
        db.collection('tarefas').doc(currentContextId).update({ status: 'informativo' });
    } else if(acao === 'excluir') {
        if(confirm(i18n[currentLang].btnDelete + "?")) db.collection('tarefas').doc(currentContextId).delete();
    }
}

// Renderizar o Mapa de Calor COM AS TRADUÇÕES APLICADAS
function abrirModalHeatmap() {
    ...
    // Cabeçalho da Tabela agora usa a chave "hmTeam"
    ...
    // Auto-scroll Inteligente: Corre exatamente para a coluna do meio-dia da semana atual
    setTimeout(() => {
        ...
        if (selAno === dayjs().year()) {
            ...
            if(elCw && container) {
                // Diminui o recuo esquerdo para deixar a semana atual quase no começo da tela
...
function salvarTarefa() {
    ...
    if(!p || !a) { alert(dict.msgEmpty); return; }
    if(!texto) { alert(dict.msgNoDesc); return; }
    if(dayjs(f).isBefore(dayjs(s))) { alert(dict.msgDateErr); return; }
...
function apagarTarefaAtual() {
    if(confirm("Confirmar?")) { db.collection("tarefas").doc(document.getElementById('i-id').value).delete(); fecharModais(); }
}
...
function adicionarProjetoConfig() {
    ...
    if(!n || !startVal || !endVal) { alert(dict.msgEmpty); return; }
    const startYear = parseInt(startVal.split('-')[0]), endYear = parseInt(endVal.split('-')[0]);
    if (endYear < startYear) { alert(dict.msgDateErr); return; }
...
function adicionarAreaConfig() {
    ...
    if(!p || !n) { alert(i18n[currentLang].msgEmpty); return; }
