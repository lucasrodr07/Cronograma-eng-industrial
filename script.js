...
db.collection("settings").doc("global").onSnapshot((doc) => {
    if (doc.exists) {
        ...
        let localCfg = JSON.parse(localStorage.getItem('motherson_config_v11')) || { dataInicio: dayjs().startOf('year').format('YYYY-MM-DD'), dataFim: dayjs().endOf('year').format('YYYY-MM-DD') };
        let localProj = JSON.parse(localStorage.getItem('motherson_projetos_v11')) || ['UDARA', 'GEELY'];
        let localArea = JSON.parse(localStorage.getItem('motherson_areas_v11')) || { 'UDARA': ['Milestones Cliente', 'Injeção', 'Montagem', 'Pintura Automática'], 'GEELY': ['Milestones Cliente', 'Injeção', 'Montagem', 'Pintura Automática'] };
        
        db.collection("settings").doc("global").set({ config: localCfg, projetos: localProj, areas: localArea });
        configGeral = localCfg; projetosLocais = localProj; areasLocais = localArea;
    }
...
db.collection("tarefas").onSnapshot((snap) => {
    if (snap.empty) {
        let localTarefas = JSON.parse(localStorage.getItem('motherson_dados_v11')) || [];
        if (localTarefas.length > 0) {
            localTarefas.forEach(t => db.collection("tarefas").doc(t.id).set(t));
        }
    } else {
        tarefas = [];
...
function showTooltip(e, id) { 
    ...
    let statusTexto = t.status === 'informativo' ? 'Operacional' : t.status === 'suporte' ? 'Suporte / Atenção' : 'Milestones Cliente';
