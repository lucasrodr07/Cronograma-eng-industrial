...
const i18n = {
    'pt-br': {
        ...
        thProj: "PROJETO", thSetor: "SETOR", alertPend: "Atenção: {n} item(ns) pendente(s) perto do prazo!"
    },
    'en': {
        ...
        thProj: "PROJECT", thSetor: "SECTOR", alertPend: "Warning: {n} pending item(s) near deadline!"
    },
    'de': {
        ...
        thProj: "PROJEKT", thSetor: "SEKTOR", alertPend: "Achtung: {n} ausstehende(s) Element(e) kurz vor Frist!"
    },
    'es': {
        ...
        thProj: "PROYECTO", thSetor: "SECTOR", alertPend: "Atención: ¡{n} elemento(s) pendiente(s) cerca de la fecha límite!"
    }
};
...
function atualizarKPIs(filtradas) {
    ...
    if(filtradas.length > 0) { 
        ...
        atrasos = filtradas.filter(t => dayjs(t.fim).isBefore(dayjs()) && (t.progresso || 0) < 100).length;
            ...
            exibe.forEach(areaNome => {
                ...
                camadas.forEach((camada, cIdx) => {
                    ...
                    dArr.forEach(d => {
                        ...
                        h += `<td class="${(d.s === 0 || d.s === 6) ? 'is-weekend' : ''} ${d.ds === HOJE ? 'is-today' : ''} ${isMonthEnd ? 'month-end' : ''}" ondragover="allowDrop(event)" ondrop="dropTask(event,'${p}','${areaNome}','${d.ds}')">`;
                        ...
                        if(task){
                            ...
                            if(task.subtarefas && task.subtarefas.length > 0) {
                                let pendentes = task.subtarefas.filter(s => !s.feita).length;
                                if (pendentes > 0 && dayjs(task.fim).diff(dayjs(), 'day') <= 3) {
                                    iconeAlerta = `<span class="task-alert" title="${dict.alertPend.replace('{n}', pendentes)}">⚠️</span>`;
                                }
                                subInfo = ` [${task.subtarefas.length - pendentes}/${task.subtarefas.length}]`;
                            }
                            
                            h += `<div class="task-wrapper ${clDimmed}" id="task-${task.id}" style="width:calc(${dur*100}% + ${dur-1}px);" onclick="handleTaskClick(event, '${task.id}')" onmousemove="showTooltip(event,'${task.id}')" onmouseleave="hideTooltip()">
...
function abrirModalAcao(id = null) {
    ...
    if(t) {
        ...
        document.getElementById('i-inicio').value = HOJE; document.getElementById('i-fim').value = HOJE;
...
function apagarTarefaAtual() { if(confirm("Deseja realmente excluir esta ação?")) { db.collection("tarefas").doc(document.getElementById('i-id').value).delete(); fecharModais(); } }
...
function importarBackup(event) {
    ...
    reader.onload = async function(e) {
        try {
            ...
            if (!backup.settings || !backup.tarefas) throw new Error("Arquivo inválido");
            if (!confirm("ATENÇÃO: A importação apagará todos os dados atuais e fará a restauração do backup. Tem certeza?")) { document.getElementById('import-file').value = ''; return; }
...
function gerarRelatorioImpressao(startStr, endStr, escala) {
            ...
            exibe.forEach(areaNome => {
                ...
                camadas.forEach((camada, cIdx) => {
                    ...
                    dArr.forEach(d => {
                        ...
                        h += `<td class="${(d.s === 0 || d.s === 6) ? 'is-weekend' : ''} ${d.ds === HOJE ? 'is-today' : ''} ${isMonthEnd ? 'month-end' : ''}">`;
                        ...
                        if(task){
                            ...
                            if(task.subtarefas && task.subtarefas.length > 0) {
                                let pendentes = task.subtarefas.filter(s => !s.feita).length;
                                if (pendentes > 0 && dayjs(task.fim).diff(dayjs(), 'day') <= 3) iconeAlerta = `<span class="task-alert">⚠️</span>`;
                                subInfo = ` [${task.subtarefas.length - pendentes}/${task.subtarefas.length}]`;
                            }
    ...
    document.getElementById('print-layout').innerHTML = `
        ...
        <div class="print-footer"><span>Motherson Group - Documento Interno de Gestão</span><span>Proud to be part of.</span></div>
    `;
}
