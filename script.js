...
// O MOTOR INTELIGENTE DE VÍNCULO MULTI-ANO
function adicionarProjetoConfig() {
    const n = document.getElementById('cfg-novo-proj').value.toUpperCase().trim();
    
    // Captura os valores de Mês/Ano (Ex: "2026-11" e "2027-03")
    const startVal = document.getElementById('cfg-proj-inicio').value; 
    const endVal = document.getElementById('cfg-proj-fim').value;

    if(!n) return alert("Por favor, digite o nome do projeto.");
    if(!startVal || !endVal) return alert("Por favor, defina o mês/ano de início e fim do projeto.");

    const startYear = parseInt(startVal.split('-')[0]);
    const endYear = parseInt(endVal.split('-')[0]);

    if (endYear < startYear) return alert("O ano de fim não pode ser menor que o ano de início.");
    
    if(!projetosGlobal[n]) projetosGlobal[n] = [];
    
    // Injeção Contínua: Adiciona o projeto em TODOS os anos do intervalo definido
    for(let y = startYear; y <= endYear; y++) {
        if(!projetosGlobal[n].includes(y)) {
            projetosGlobal[n].push(y);
        }
    }
    ...
    document.getElementById('cfg-proj-inicio').value = '';
    document.getElementById('cfg-proj-fim').value = '';
}

function removerP(n) {
    const selAno = parseInt(document.getElementById('filtro-ano-view').value) || Math.max(2026, dataSistema.year());
    if(confirm(`Remover o projeto ${n} do ano de ${selAno}? (O projeto continuará existindo nos outros anos de sua duração)`)) {
