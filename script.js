...
db.collection("settings").doc("global").onSnapshot((doc) => {
    if (doc.exists) {
        ...
        let localCfg = { dataInicio: dayjs().startOf('year').format('YYYY-MM-DD'), dataFim: dayjs().endOf('year').format('YYYY-MM-DD') };
        db.collection("settings").doc("global").set({ config: localCfg, projetos: ['UDARA'], areas: {'UDARA':['Injeção']} });
    }
